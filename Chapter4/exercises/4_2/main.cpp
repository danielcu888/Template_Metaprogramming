...
namespace ex_4_2
{
  template< typename T
          , typename Q
          , typename A = std::true_type
          , typename B = std::true_type
          , typename C = std::true_type
          >
  struct logical_or_
    : boost::mpl::eval_if_c<
       T::type::value
...
int main(int argc, char *argv[])
{
  ...
  static_assert(ex_4_2::is_one_or_two<boost::mpl::int_<1>>::value, "Should be 1");

  // Test for boost::mpl::and_ (uses short-circuit because fails first condition).
  static_assert(!ex_4_2::is_one_and_two<boost::mpl::int_<3>>::value, "Should not be 1 or 2");
